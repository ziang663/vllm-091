...
from vllm.v1.core.async_kv_cache_manager import AsyncKVCacheManager
...
class Scheduler(SchedulerInterface):
    ...
    def __init__(
        ...
        self.preschedule_step = 0
        self.output_update_step = 1
        self.pre_num_computed_tokens: dict[str, int] = {}

        self.schedule_condition = threading.Condition()
        self.preschedule_condition = threading.Condition()
        self.odd_output: SchedulerOutput
        self.even_output: SchedulerOutput        
        ...
        self.kv_cache_manager = AsyncKVCacheManager(
        ...
        threading.Thread(target=self.preschedule_loop,
                         daemon=True).start()

    def preschedule_loop(self) -> None:
        while True:
            while not self.has_unfinished_requests():
                continue
            with self.preschedule_condition:
                # If preschedule_step == schedule_step + 1, preschedule_loop
                # should wait for execution of current schedule_step since
                # there is no need for multi-step pre-schedule.
                self.preschedule_condition.wait_for(
    ...
    def _preschedule(self) -> None:
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            prenum_computed_token = request.num_computed_tokens
            prenum_tokens_with_spec = request.num_tokens_with_spec
            if self.preschedule_step == self.output_update_step:
                # TODO: may not correct here.
                if prenum_computed_token >= request.num_prompt_tokens: 
                    prenum_tokens_with_spec += 1       
            ...
            if request.has_encoder_inputs:
                (encoder_inputs_to_schedule, num_new_tokens,
                 new_encoder_budget) = self._try_schedule_encoder_inputs(
                     request, prenum_computed_token, num_new_tokens,
                     encoder_budget)
        ...
        if self.preschedule_step % 2 == 0 :
            self.even_output = scheduler_output
        else : 
            self.odd_output = scheduler_output
    ...
    def update_from_output(
        ...
        with self.schedule_condition:
            self.schedule_condition.wait_for(
                lambda: self.preschedule_step > self.output_update_step)
